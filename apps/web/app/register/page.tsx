  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth-proxy/register", {
        ...
        body: JSON.stringify({ email: email.trim(), password, name: pseudo.trim() || undefined }),
      });
    ...
    <main className="login">
      <div className="login__container">
        <div className="login__header">
          ...
          <p className="login__subtitle">Créez votre compte pour commencer.</p>
        </div>
        ...
        <div className="login__providers">
          ...
          >
            <svg className="login__btn-icon" viewBox="0 0 24 24" width="20" height="20">
              <path
                fill="currentColor"
        ...
        <form className="login__credentials-form" onSubmit={handleSubmit}>
          ...
          >
            {loading === "credentials" ? "Inscription..." : "Créer mon compte"}
          </button>
          <div className="login__links" style={{ justifyContent: "center" }}>
            <a href="/login" className="login__link">Déjà un compte ? Se connecter</a>
          </div>
        </form>
      </div>
    </main>
